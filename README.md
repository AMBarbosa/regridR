...
url <- "https://sdi.eea.europa.eu/datashare/s/jokLrYcEBFiJRyo/download?path=%2FGPKG&files=EEA_50km_grid_v2024.gpkg"

eea50 <- terra::vect(url)
...
layers_regrid <- regridR::regrid(layers = layers, grid = eea50, 
                                 na.rm = TRUE, touches = TRUE)

terra::plot(layers_regrid[[1:4]])
```
...
#> Cannot preload entire working area of 548441998 cells with max_cells_in_memory = 3e+07. Raster values will be read for each feature individually.
...
We can plot a smaller area to visually check that the output
(re-gridded) `layers`’ pixels align with the input `grid`:

``` r
terra::plot(layers_regrid[[1]], ext = c(2.3e+06, 2.8e+06, 1.8e+06, 2.2e+06))

terra::plot(eea50, add = TRUE)
```
